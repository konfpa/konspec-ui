...
window.SPEC = {
  ...
  searchAliases: {
    ...
    'index-page': 'list register index screen table page browse',
    'record-page': 'detail record view show entity screen',
    dashboard: 'home overview kpi metrics landing summary',
    board: 'kanban columns stages pipeline swimlane',
    'document-page': 'print invoice challan gst pdf paper a4 letterhead',
    container: 'width gutter spacing grid stack layout wrapper max-width',
    'inbox-page': 'approvals queue triage worklist pending sign-off',
    'focus-page': 'kiosk scan barcode shopfloor fullscreen tablet count',
    'import-page': 'upload csv excel bulk mapping migrate spreadsheet',
    'compare-page': 'comparison quotation matrix vendors l1 side-by-side award',
    'reconcile-page': 'match three-way variance ledger statement settle gap',
    'search-page': 'results facets global find query lookup',
    'schedule-page': 'calendar planner gantt timeline month week shifts',
    'audit-page': 'trail history log activity changes who-changed diff',
    'label-sheet': 'barcode qr sticker bin batch pallet print roll',
    'docs-page': 'documentation help sop manual guide article knowledge-base',
    'portal-page': 'vendor supplier external customer extranet co-branded',
    'share-page': 'unlisted link public download recipient external anonymous no-login',
    'onboarding-page': 'setup checklist first-run getting-started deployment',
    'settings-page': 'preferences configuration admin options masters',
  ...
  protocol: [
    ...
    { t: 'Fetch one variant at a time and copy verbatim',
      d: 'The response body of a variant endpoint is the markup itself: no JSON envelope, no escaping, nothing to parse. Copy the string. Do not paraphrase markup you have already been handed, and do not reformat it into your own idiom. llms.txt lists every component with its variant ids, so once you have read it you can build these URLs yourself and never need an index. Fetch r/<id>.json when you need a component\'s rules, anatomy or accessibility notes rather than its markup: it carries no html at all, so it is a few thousand tokens rather than the tens of thousands it used to be, and it is also where the variant names live when an id alone does not tell you which one to take. Fetch r/index.json only if you arrived without reading llms.txt.',
      code: 'GET /r/<id>/<variant>.html  →  the markup, ready to paste' },
    { t: 'Fetch the smallest thing that answers the question',
      d: 'Three sizes, and the gap between them is wide enough that taking the wrong one costs more than the answer is worth. A variant is the markup and nothing else, a thousand tokens or two, and it is the only one of the three you need in order to build a screen. r/<id>.json is the component with its markup taken out — the rules, the anatomy, the behaviour, the accessibility notes and the name of every variant — and you fetch it when you need to know why a component is shaped the way it is, or which of its variants to take. r/index.json is every component summarised in one file, and it is for an agent that arrived here without reading llms.txt. registry.json is none of these any more: it was the entire system in one response, several times a context window, and it now returns a short pointer at these endpoints instead. It had to stop being the whole file because an agent that fetched it got a truncated one, and a truncated registry is worse than none — the components above the cut look complete and the ones below it look absent, and nothing in the response says which is which.',
      code: 'GET /r/<id>.json  →  rules and variant names, no markup' },
  ...
  rules: [
    ...
    { t: 'Every signed-in screen is app-shell',
      d: 'This is the one layout decision that is already made, and it is not a default you may weigh against alternatives. If a signed-in user can reach the page, the page is app-shell with the screen\'s own content inside <main>, copied from /r/app-shell/default.html like any other component. There are five exceptions and no more. Two are about context the shell cannot get: auth-page and error-page, because the shell reads the navigation, the counts and the signed-in user out of context that a signed-out visitor or a failed request may not have. Two are about a user the shell was not built for: portal-page, where the reader is an external supplier and the nav, the counts and the internal identity do not exist for them at all; and focus-page, the shop-floor screen where the chrome is not merely useless but harmful, because a rail takes target area away from a gloved hand and a nav link is a route to losing an uncommitted count. The fifth is share-page, and it is a step further out than either pair: its reader has no identity of any kind, ever, not expired and not external, just never issued one, because the page is reached from an unlisted link rather than a sign-in of any shape. The test for all but the first two is narrow and you should expect to fail it: the shell has to be actively wrong for this reader, not merely unnecessary. A screen that simply feels cluttered is app-shell. Do not assemble a shell of your own out of sidebar and topbar. Those entries document the shell\'s parts and exist so its own variants can be built; a hand-rolled sidebar-plus-topbar is a second implementation of app-shell, and two implementations of one thing drift apart while only one of them keeps the accessibility work. Do not ship a lighter shell for one screen, and do not start a page at <main> because the screen looked simple enough not to need chrome. The shell owns the skip link, the nav landmark, the off-canvas focus trap, the collapse persistence, the command palette and the keyboard shortcuts; a page that skips it loses every one of them silently and still reviews as fine, because everything it dropped is invisible until somebody uses a keyboard or a phone. The test is the response, not the screen: if a signed-in request returns a page, it is app-shell unless it is one of the five named above. An htmx partial swapped into a shell already on screen is not a page and must not repeat it.' },
    ...
    { t: 'Never bind Alpine on a Lucide icon',
      d: 'createIcons() replaces the <i> with an <svg>, destroying any :class on it. Bind on a wrapping span instead. The same class of bug bites one step further out: Alpine removes only the classes it added, so a class written into the markup on an element whose class is also bound survives every state change — a scroll-spy rail written that way marks two headings at once, forever. And a state class with no colour of its own, border-l-2 with no border-*, renders in currentColor until Alpine boots, which on a slow page is a dark rule down every item and on a page where Alpine fails is permanent. A marker element that is transparent at rest and carries no static class is the shape that cannot fail either way.' },
    ...
    { t: 'Every tinted shape carries its ring',
      d: 'A tinted shape is bg-zinc-200 with ring-1 ring-inset ring-zinc-300 — pills, chips, avatars, icon wells, control tracks, the selected nav item, all of them. A tinted surface is bg-zinc-100 — a selected row, the highlighted option in a menu — the page is the same step one ramp over at bg-stone-100, and a band recessed inside a white surface is bg-zinc-50, which is a table header or a zebra row. Give a shape the surface fill and it measures 1.00 contrast against the surface it sits on, which is not low contrast but the identical colour, and it disappears. One step of fill separates them and the ring holds the edge. Solid shapes need no ring, and it is ring rather than border so adding it reflows nothing. One exception, and only one: inline code inside a sentence takes the tint without the ring. A ring drawn around two words mid-paragraph cuts the line of the text it sits in, and the shape is already separated from the prose by the mono face.' },
    { t: 'Hover never lands on the surface behind it',
      d: 'A hover fill that equals the fill underneath it is not a subtle hover, it is no hover: the control loses its surface and the one thing the cursor is on reads as less present than the things it is not on. The page is bg-stone-100, so a bordered white button sitting on the page cannot hover to zinc-100 \u2014 the two are a different ramp but the same value, so it dissolves and leaves its border behind exactly as it would have on a matching hex. The lint below can no longer catch that one by equality, which is why stone is in its step table by name. Two kinds of interactive thing, and the split is the same one the tinted-shape rule draws. A SHAPE is self-sized and rounded on every corner: a button, an icon button, a menu trigger, a chip. It hovers to the chip fill zinc-200 wherever it sits, because a shape has to read against whatever it is dropped onto \u2014 white if it rests white on a white card, and one step further to zinc-300 if it is already sitting on a zinc-200 track. A SURFACE is a full-bleed band: a table row, a menu item, a sidebar link, a member of a joined strip whose corners are shared with its wrapper. It steps once off the band it crosses, so zinc-100 inside a white panel and zinc-200 on the page. Anything with a solid fill of its own ignores all of this and steps one deeper: zinc-700 to zinc-800, red-600 to red-700, zinc-200 to zinc-300. node tools/build.js checks every variant against this and fails the build rather than warning, so the rule is not something anyone has to remember at the call site.' },
    { t: 'A region may scroll sideways. The page may not',
      d: 'Nothing in this system produces a horizontal scrollbar on the document at 390px — that is the rule the build cannot check and every review has to. What a wide thing does instead is scroll inside its own box: a comparison matrix, a rate grid, a board of stages, a printed line table. Three things come with that box and none of them is optional. It carries relative, because an sr-only span inside a scroller has no positioned ancestor otherwise, resolves against the document, escapes the clip and widens the whole page — this has now been the cause of a 390px overflow in four separate components and it is invisible in review because the element it comes from is invisible. It carries role="region", tabindex="0" and an accessible name, because Chromium will not give keyboard focus to an overflow container on its own and a matrix nobody can scroll without a mouse is a matrix half the users cannot read. And below the width at which it starts to clip it carries a line of text saying so, at text-[12px]/4 text-zinc-500 — there is no gradient, no fade and no arrow in this system, so the affordance is a sentence.' },
    ...
    { t: 'Paper is a medium, not a breakpoint',
      d: 'Two entries print: document-page for A4 paperwork and label-sheet for die-cut label stock. What they established holds for anything else that ever does. Screen chrome carries print:hidden and the document is what is left. Backgrounds do not print, so nothing may carry meaning in a fill — which means a printed document has no status pill at all, because the 6px dot is a background circle and on paper the state has to become a word. A watermark is the same mistake twice. Structure comes from borders, and hierarchy from weight rather than from a darker grey: border-zinc-300 is about 16% grey on paper and a thermal printer, which is one bit deep, dithers it. Do not trust a breakpoint to mean paper — A4 portrait is 794 CSS px so md: applies and lg: does not, but the print dialog\'s Scale field changes the layout width, and at 50% every lg: rule switches on; where the printed shape must differ, write print:. Put break-inside-avoid on every row, on a totals block and on a signature block, because a total split across two sheets is a defect. Nothing inside a sheet is sticky: it prints on the first page and leaves a hole on the rest. Page numbers and carried-forward subtotals come from the view, because counter(page) in an @page margin box is not implemented in Chrome and no CSS can sum the previous sheet. And a barcode is an <img> whose src the server renders — not a font, which cannot do QR error correction, and not a JavaScript library, which draws after paint and is therefore not there when window.print() fires, so the sheet comes out blank exactly where every symbol should be. @page itself is an at-rule and can never be a Tailwind utility: it is the one piece of hand-written CSS this system needs, it goes in the project stylesheet beside the x-cloak rule, and because the last @page parsed wins for every printed page in the document, a deployment that prints both A4 paperwork and 100x50mm labels needs one print base per medium rather than one stylesheet for both.' },
  ...
  tokens: {
    surfaces: [
      ['Page',        'bg-stone-100', 'The scrolling page behind everything. Nothing inside a white surface may take this fill — same hex twice is a 1.00 contrast, and the card reads as a hole.'],
      ...
      ['Edge',        'border-zinc-300', 'The outer edge of anything sitting on the page. White on stone-100 is 1.10 and a zinc-200 edge is the first thing a washed-out panel drops.'],
    ...
    type: [
      ...
      ['text-[32px]/9', 'the one value read at arm\'s length on a shop-floor screen — a scanned code, a counted quantity. focus-page only, and never more than one per screen']
    ],
    weight: [
  ...
  status: [
    { s: 'Draft',     pill: 'bg-zinc-200 text-zinc-600 ring-1 ring-inset ring-zinc-300', dot: 'bg-zinc-400',    why: 'Not real yet — nothing has been submitted' },
    { s: 'Cancelled', pill: 'bg-zinc-200 text-zinc-600 ring-1 ring-inset ring-zinc-300', dot: 'bg-zinc-400',    why: 'Ended without completing. Inert, like a draft — not emerald, because it is over but that is not good news' },
    { s: 'Open',      pill: 'bg-zinc-200 text-zinc-700 ring-1 ring-inset ring-zinc-300', dot: 'bg-zinc-500',    why: 'Live, and nothing is wrong with it' },
    { s: 'Pending',   pill: 'bg-zinc-200 text-zinc-700 ring-1 ring-inset ring-zinc-300', dot: 'bg-amber-500',   why: 'Awaiting somebody\'s decision — this is the word for an item sitting in an approval inbox' },
    { s: 'Approved',  pill: 'bg-zinc-200 text-zinc-700 ring-1 ring-inset ring-zinc-300', dot: 'bg-amber-500',   why: 'Approved, and now waiting on the vendor to deliver. It does NOT mean awaiting approval — that is Pending' },
    { s: 'On hold',   pill: 'bg-zinc-200 text-zinc-700 ring-1 ring-inset ring-zinc-300', dot: 'bg-amber-500',   why: 'Stopped on purpose, waiting on a person to release it' },
    { s: 'Overdue',   pill: 'bg-zinc-200 text-zinc-700 ring-1 ring-inset ring-zinc-300', dot: 'bg-red-600',     why: 'The alarm state — past its date, failed, or refused' },
    { s: 'Closed',    pill: 'bg-zinc-200 text-zinc-700 ring-1 ring-inset ring-zinc-300', dot: 'bg-emerald-600', why: 'Finished, and finished well' }
  ],

  /* Six entries in two batches reached for a status word this table did not carry —
     a reconciliation verdict, a QC outcome, a delivery that was cancelled. Each one
     correctly refused to invent a colour and each one had to guess. The table cannot
     grow a row per word, so the meanings are stated once and any word maps onto them. */
  statusMeanings: {
    d: 'The five dots are meanings, not words. A state this table does not list takes the dot whose meaning matches, and never a new colour.',
    map: [
      ['bg-zinc-400',    'Inert. Not real yet, or over without completing. Draft, Cancelled, Void, Superseded.'],
      ['bg-zinc-500',    'Live and ordinary. Nothing is wrong and nobody is waiting. Open, In progress, Carried forward.'],
      ['bg-amber-500',   'Waiting on somebody before it can move. Pending, Approved, On hold, Needs a reason, With finance.'],
      ['bg-red-600',     'Stopped, failed or past its date. Overdue, Rejected, Blocked, Failed inspection, Short received.'],
      ['bg-emerald-600', 'Finished and correct. Closed, Received in full, In tolerance, Passed.']
    ],
    note: 'A verdict about a figure — In tolerance, Needs a reason, Blocked — is not a record state and does not belong in the table above, but it takes its dot from the same five meanings. Amber is a 6px disc at -500 and a 1.5px stroke at -700; that is the one place the same meaning takes two shades.'
  },
